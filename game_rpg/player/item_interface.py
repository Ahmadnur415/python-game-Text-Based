...
def item_interface(player, item):
    while True:
        ...
        elif index == namespace.USE_ITEMS:
            player.equip_items_interface(item)

        elif index == namespace.SELL_ITEMS:
            player.sell_item(item)
            if item.amount < 1:
                return

        elif index == namespace.CONSUME_ITEMS:
            player.consume_item(item)
            interface.print_("\n")
            if item.amount < 1:
                return

        elif index == namespace.BACK:
            return index
